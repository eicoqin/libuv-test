...
void xx::UvTcpUdpBase::ReceiveImpl(char const* bufPtr, int len)
{
	...
	while (offset + 3 <= bbRecv.dataLen)			// 确保 3字节 包头长度
	{
		...
		if ((typeId & 8) > 0)                       // 转发类数据包
		{
			...
			addrLen = (size_t)(buf[addrOffset]) + 1;
			...
			{
				//auto addrSize = (size_t)(buf[addrOffset])+1;
				//addrLen = addrSize;
				// 存返件地址之后 跳到正常包逻辑代码继续处理
				recvingAddress.Assign((char*)buf + addrOffset , addrLen);//string头自带长度
	...
	LabAfterAddress:
		...
		if (pkgType == 0)
		{
			if (OnReceivePackage) OnReceivePackage(bbRecv,typeId);
			if (versionNumber != memHeader().versionNumber) return;
			if (Disconnected())
			{
				bbRecv.Clear();
		...
		else
		{
			...
			if (pkgType == 1)
			{
				if (OnReceiveRequest) OnReceiveRequest(serial, bbRecv, typeId);
				if (versionNumber != memHeader().versionNumber) return;
				if (Disconnected())
				{
					bbRecv.Clear();
